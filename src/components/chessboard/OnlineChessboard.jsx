...
function OnlineChessboard({ reset_prop }) {
  ...
  const getValidPositions = (piece, position) => {
    ...
    if (piece.pieceId == 0) {
      if (piece.color == 0) {
        if (row == 6) {
          if (
            all_Elements.list_elements[row - 2][col].piece == null &&
            all_Elements.list_elements[row - 1][col].piece == null
          ) {
            addMove(row - 2, col);
          }
        }
        ...
        if (
          col >= 1 &&
          all_Elements.list_elements[row - 1][col - 1].piece != null
        ) {
          addMove(row - 1, col - 1);
        }
        if (
  ...
  const CheckValidPlaces = (this_row, this_col, this_pieceId, this_color) => {
    ...
    let valid_Positions = getValidPositions(
      { pieceId: this_pieceId, color: this_color },
    ...
    for (let v in valid_Positions) {
      dispatch(
        updateValidElement({
          row: valid_Positions[v].row,
          col: valid_Positions[v].col,
          valid: true,
        }),
      );
    }
  ...
  return gameOver ? (
    ...
    <>
      <div className="PlayerInfo left-[15rem]">
        {curr_user.curr_user === 1 ? "WHITE TURN" : "BLACK TURN"}
      </div>
      <div className="killStore whiteK absolute left-[15rem] top-[19vh]">
        {
          //kill list:
          killStore.killed_elements.white.map((element, index) => {
            return (
              <div
                className={`piece ${element.color === 0 ? "black" : "white"} piece-${element.pieceId}`}
      ...
      <div className="chessboard absolute left-[15rem]">
      ...
      <div className="killStore blackK absolute left-[15rem]">
        {
          //kill list:
      ...
      <div className="Player1 w-[60rem] h-[18rem] absolute top-[19vh] right-[19vh] bg-red-300 border-[1vh] border-[rgb(18,24,38)] flex">
        <div className="video w-[55%] h-full bg-violet-300">Moves</div>
        <div className="video w-[45%] h-full bg-blue-300 border-l-[1vh] border-black ">
          Video
        </div>
      </div>
      <div className="Player2 w-[60rem] h-[18rem] absolute top-[51vh] right-[19vh] bg-red-300 border-[1vh] border-[rgb(18,24,38)] flex">
        <div className="video w-[55%] h-full bg-violet-300">Moves</div>
        <div className="video w-[45%] h-full bg-blue-300 border-l-[1vh] border-black ">
          Video
        </div>
